...
<nav x-data="{ open: false }"
    class="{{ $isHero ? 'absolute inset-x-0 top-0 z-50' : '' }} {{ $isHero ? '' : 'bg-white border-b border-gray-100' }}">
    <!-- Primary Navigation Menu -->
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            <!-- Left links -->
            <div class="flex items-center space-x-6">
                <p class="text-md font-medium text-tanean-dark">{{ ucfirst(Auth::user()->name) }}</p>
            </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 space-x-4">
                @auth
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button
                                class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md {{ $isHero ? 'text-white bg-transparent' : 'text-gray-500 bg-white' }} hover:opacity-90 focus:outline-none transition ease-in-out duration-150">
